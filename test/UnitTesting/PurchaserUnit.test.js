...
const service=require('../../src/Service/PurchaserService');
const model=require('../../src/Model/PurchaserModel');
const httpMocks=require("node-mocks-http");
const mockPurch=require("../MockData/PurchaserData.json");
...
/**
 * Unit test for getPurchaserDetails :
 */
describe("service.getPurchaserDetails",()=>{

it("Should have a getPurchaserDetails function",()=>{
expect(typeof service.getPurchaserDetails).toBe("function");
});

it("Should call model.find{}",async()=>{
   await service.getPurchaserDetails(req,res,next);
...
it("Should return response as 404 when data not found",async()=>
{
...
/**
 * Unit test for addPurchaserDetails :
 */
describe("service.addPurchaserDetails",()=>{

it("Should have a addPurchaserDetails function",()=>{
    expect(typeof service.addPurchaserDetails).toBe("function");
    });

it("Should call model.create",async ()=>{
    service.addPurchaserDetails(req,res,next);
    expect(model.create).toBeCalledTimes(1);
    });


it("Should return 200 as response status",async()=>{
    await service.addPurchaserDetails(req,res,next);
    expect(res.statusCode).toBe(200);
    });
...
it("Should call model.find({})",async()=>{
   ...
   expect(model.find).toBeCalledWith({purchaserApproval:{$eq:"Approved"}});
...
it("Should return 404 when item data not found",async()=>
   {
   model.find.mockReturnValue(null);
   await service.getApprovedDetails(req,res,next);
   expect(res.statusCode).toBe(404);
...
it("Should call model.find({})",async()=>{
      ...
      expect(model.find).toBeCalledWith({purchaserApproval:{$eq:"Rejected"}});
...
it("Should do error handling",async()=>{
   const errorMessage={message:"error in deleting data"};
   const rejectedPromise=Promise.reject(errorMessage);
